...
def parse_args():
    ...
    parser.add_argument('-m', '--model', help='Trained model',
                        dest='model',
                        default='../model/TransTCR/model_head1_fold4_epoch1.pkl',
                        type=str
                        )
...
def eval_step(model, loader, use_cuda=True, name='test'):
    ...
    with torch.no_grad():
        ...
        metrics_val = performances(y_true_val_list, y_pred_val_list, y_prob_val_list, print_=False)


    return ys_val, loss_val_list,metrics_val

def neg_eval_step(model, loader, use_cuda=True, name='test'):
    device = torch.device("cuda" if use_cuda else "cpu")
...
ys_test_val, loss_test_val_list,metrics_val = eval_step(model,test_loader, use_cuda,'test')
...
for index, (pep, hla, prob) in enumerate(zip(df['peptide'], df['HLA_sequence'], df['prob'])):
    ...
    test_tcr_inputs, test_pep_inputs, test_hla_inputs, test_loader = Test_DataProcessing.pMHCI_TCR_test_data_with_loader(
        df_create, vocab, batch_size=1024, pep_max_len=25, hla_max_len=34, tcr_max_len=27, shuffle=False)

    ys_test_val, loss_test_val_list = neg_eval_step(model, test_loader, use_cuda, 'test')

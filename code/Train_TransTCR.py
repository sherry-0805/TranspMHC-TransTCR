...
def train_step(model, optimizer, train_loader, fold, epoch, epochs, use_cuda=True):
    ...
    for train_postcr_inputs, train_pep_inputs, train_hla_inputs, train_negtcr_inputs, train_labels in tqdm(
        ...
        optimizer.zero_grad()
        train_pos_outputs, _, _,  train_pos_dec_self_attns = model(train_postcr_inputs, train_pep_inputs,
                                                                 train_hla_inputs)
        train_neg_outputs, _, _,  train_neg_dec_self_attns = model(train_negtcr_inputs, train_pep_inputs,
                                                                 train_hla_inputs)
        y_pos_tcr = train_pos_outputs[:, 1]
        y_neg_tcr = train_neg_outputs[:, 1]
        df_loss = differential_loss( y_pos_tcr, y_neg_tcr)


        weights = torch.tensor([1.0, 2]).to(device)

        criterion = nn.CrossEntropyLoss(weight=weights)

        pos_label = torch.LongTensor([1] * train_pos_outputs.size(0)).to(device)
        neg_label = torch.LongTensor([0] * train_neg_outputs.size(0)).to(device)
        pos_neg_outputs = torch.cat((train_pos_outputs, train_neg_outputs), dim=0)
        pos_neg_label = torch.cat((pos_label, neg_label), dim=0)

        ce_loss = criterion(pos_neg_outputs, pos_neg_label)


        weight_l2 = 1e-5
        l2_norm = sum(p.pow(2.0).sum() for p in model.parameters())
        l2_reg = weight_l2 * l2_norm

        alpha = 1.0
        beta = 1

        train_loss = alpha * ce_loss + beta * df_loss + l2_reg
...
# Drop duplicate rows
train_posdata = train_posdata.drop_duplicates()
...
train_postcr_inputs, train_pep_inputs, train_hla_inputs, train_negtcr_inputs, train_labels, train_loader = pMHCI_TCR_train_data_with_loader(
    train_posdata, train_negdata, vocab, batch_size=1024, pep_max_len=25, hla_max_len=34,tcr_max_len=27,shuffle=True)
test_tcr_inputs, test_pep_inputs, test_hla_inputs, test_labels, test_loader = pMHCI_TCR_test_data_with_loader(
    test_data, vocab, batch_size=1024,pep_max_len=25,hla_max_len=34,tcr_max_len=27, shuffle=False)
...
for n_heads in range(1,9):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(train_postcr_inputs)):
        ...
        train_postcr, train_pep, train_hla, train_label = (
            np.repeat(train_postcr_inputs[train_index], 3, axis=0),
            np.repeat(train_pep_inputs[train_index], 3, axis=0),
            np.repeat(train_hla_inputs[train_index], 3, axis=0),
            np.repeat(train_labels[train_index], 3, axis=0)
        )
        num_negtcr_samples = len(train_postcr)
        negtcr_indices = np.random.choice(len(train_negtcr_inputs), num_negtcr_samples, replace=False)

        train_negtcr = train_negtcr_inputs[negtcr_indices]
        ...
        unique_train_labels, train_label_counts = torch.unique(train_label, return_counts=True)
        print("Train labels distribution:", dict(zip(unique_train_labels.tolist(), train_label_counts.tolist())))

        val_postcr, val_pep, val_hla, val_label = (
            train_postcr_inputs[val_index],
            train_pep_inputs[val_index],
            train_hla_inputs[val_index],
            train_labels[val_index]
        )
        num_negtcr_samples = len(val_postcr)*3
        negtcr_indices = np.random.choice(len(train_negtcr_inputs), num_negtcr_samples, replace=False)

        val_negtcr = train_negtcr_inputs[negtcr_indices]

        val_tcr_inputs = torch.cat((val_postcr, val_negtcr), dim=0)
        val_pep_inputs = torch.cat([val_pep] * 4, dim=0)
        val_hla_inputs = torch.cat([val_hla]*4 , dim=0)
        # Create labels for the validation set
        pos_label = torch.LongTensor([1] * val_postcr.size(0))
        neg_label = torch.LongTensor([0] * val_negtcr.size(0))
        val_label_inputs = torch.cat((pos_label, neg_label), dim=0)
        val_loader  = Data.DataLoader(pMHCI_TCR_test_dataset(val_tcr_inputs, val_pep_inputs, val_hla_inputs, val_label_inputs), batch_size, shuffle=False,num_workers=0)


        unique_val_labels, val_label_counts = torch.unique(val_label_inputs, return_counts=True)
        print("Validation labels distribution:", dict(zip(unique_val_labels.tolist(), val_label_counts.tolist())))

        """ Load model """
        model = TransTCR(use_cuda = use_cuda, tgt_len=tgt_len,d_model=d_model).to(device)
        optimizer = optim.Adam(model.parameters(), lr=0.25*1e-3)
